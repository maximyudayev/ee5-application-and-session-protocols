 ...
 * Private Prototypes
 **/
static void * HTTP_client_routine(void * arg);
static void print_help(void);
static int HTTP_msg_num;
static int HTTP_msg_period;
...
int main(int argc, char *argv[])
{
    ...
    {
        HTTP_clients = atoi(argv[1]);
        HTTP_msg_num = atoi(argv[2]);
        HTTP_msg_period = atoi(argv[3]);
    }
    ...
    // Parent's Code - X threads to run as HTTP clients
    ...
    pthread_t threads[HTTP_clients];
    void * exit_codes[HTTP_clients]; // array of pointers to thread returns
    int clients[HTTP_clients];

    for(int i = 0; i < HTTP_clients; i++)
    { 
        clients[i] = i;
        pthread_create(&(threads[i]), NULL, &HTTP_client_routine, &(clients[i]));
    }

    for(int i = 0; i < HTTP_clients; i++) pthread_join(threads[i], &exit_codes[i]); // blocks until all threads terminate
    ...
    for(int i = 0; i < HTTP_clients; i++) free(exit_codes[i]); // release memory alloc'ed to exit codes
...
static void * HTTP_client_routine(void * arg)
{
    ...
    *retval = EXIT_SUCCESS;
    ...
    #if (DEBUG_LVL > 0)
    printf("HTTP client #%d online\n", *((int *) arg));
    fflush(stdout);
    #endif

    for(int i = 0; i < HTTP_msg_num; i++)
    {
        printf("HTTP client #%d sending message %d\n", *((int *) arg), i);
        system("curl 192.168.0.226:80/hello");
        sleep(HTTP_msg_period);
    }

    #if (DEBUG_LVL > 0)
    printf("HTTP client #%d is stopped\n", *((int *) arg));
    fflush(stdout);
    #endif
...
static void print_help(void)
{
    ...
    printf("\t%-15s : Number of HTTP client threads\n", "\'HTTP_clients\'");
    printf("\t%-15s : HTTP total message number\n", "\'HTTP_msg_num\'");
    printf("\t%-15s : HTTP message periodicity\n", "\'HTTP_msg_period\'");
    fflush(stdout);
}